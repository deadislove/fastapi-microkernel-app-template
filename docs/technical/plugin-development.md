...
- **`service.py`/`router.py` may import `service_registry`/`event_bus` directly** to call `.resolve(...)`/`.emit(...)`. Neither of those registers anything or needs an `owner` tag, so there's nothing unsafe about it — see `product_plugin/service.py`'s `event_bus.emit("product.created", ...)` call for a real example. This is a deliberate scope boundary, not an oversight: hot-reload needs to revoke exactly one plugin's published capabilities/subscriptions without touching anyone else's, which only `provide()`/`subscribe()` (the calls that register something) need an `owner` tag for — `resolve()`/`emit()` only read or fire an already-registered thing, so scoping them would add ceremony without giving hot-reload anything to act on. See the "Scope of the `ctx`-only rule" section of `ServiceRegistry`'s docstring in `app/core/registry.py` for the full reasoning.
