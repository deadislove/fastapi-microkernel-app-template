...
**Test the scaffold's output without touching the filesystem.** `scripts/new_plugin.py`'s `_render(name)` is a pure function returning `{filename: content}` — `test_scaffold.py` calls it directly and `compile()`s each result, instead of running the script and asserting on files it wrote into `app/plugins/`. When the scaffold tool was manually verified end-to-end (generating a real plugin into a scratch copy of the repo, running its generated test, running the full suite), that was a one-off manual check, not something to leave as a permanent test — see [`docs/spec/done/microkernel-architecture-improvements.md`](../spec/done/microkernel-architecture-improvements.md) S3.14 for that trace. The same applies to any one-off "does the mechanism actually work end-to-end" verification: do it, record the result, then remove the scratch artifact rather than leaving throwaway code in the suite.
